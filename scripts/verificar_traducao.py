...
PARES = [
    ("_source/en-quarto/episodios/01-matrizes-de-contato.qmd",
     "episodios/01-matrizes-de-contato.qmd"),
    ("_source/en-quarto/episodios/02-simular-transmissao.qmd",
     "episodios/02-simular-transmissao.qmd"),
    ("_source/en-quarto/episodios/03-escolher-modelo.qmd",
     "episodios/03-escolher-modelo.qmd"),
    ("_source/en-quarto/episodios/04-modelar-intervencoes.qmd",
     "episodios/04-modelar-intervencoes.qmd"),
    ("_source/en-quarto/episodios/05-comparar-intervencoes.qmd",
     "episodios/05-comparar-intervencoes.qmd"),
    ("_source/en-quarto/episodios/06-comparar-vacinacao.qmd",
     "episodios/06-comparar-vacinacao.qmd"),
    ("_source/en-quarto/episodios/07-carga-de-doenca.qmd",
...
ADICOES_DOCUMENTADAS = {"options(repos = c(CRAN = \"https://cran-r.c3sl.ufpr.br/\"))"}

# arquivos cujo texto legitimamente contém muitos termos em inglês (heurística desligada)
SEM_HEURISTICA = {"glossario.qmd", "setup.qmd", "perfis.qmd"}
...
def eh_mermaid(codigo: str) -> bool:
    primeira = codigo.strip().split("\n")[0].strip().lower()
    return primeira.startswith("flowchart") or primeira.startswith("graph")


def esqueleto_mermaid(codigo: str) -> str:
    """Só a estrutura do diagrama: ids dos nós, setas, classes.

    O texto dos rótulos é prosa e PODE ser traduzido (é a figura que o aprendiz lê);
    o que não pode mudar é a sintaxe. Então comparamos o esqueleto sem rótulos.
    """
    linhas = []
    for linha in codigo.split("\n"):
        l = linha.strip()
        if not l:
            continue
        if l.lower().startswith(("flowchart", "graph")):
            linhas.append(l.split()[0].lower())
            continue
        if l.startswith(("accTitle:", "accDescr:")):
            linhas.append(l.split(":", 1)[0])       # só a chave: o texto é prosa
            continue
        if l.startswith(("classDef", "class ", "%%")):
            linhas.append(l)
            continue
        l = re.sub(r'"[^"]*"', '""', l)
        l = re.sub(r"\|[^|]*\|", "||", l)
...
def checar(en_rel: str, pt_rel: str) -> list:
    ...
    # blocos Mermaid: o rótulo é prosa (traduzível), a sintaxe não. Compara-se o esqueleto.
    en_norm = [esqueleto_mermaid(b) if eh_mermaid(b) else normalizar(b) for b in en_blocos]
    pt_norm = [esqueleto_mermaid(b) if eh_mermaid(b) else normalizar(b) for b in pt_blocos]
