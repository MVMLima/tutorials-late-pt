...
+ Complete tutorials [Simulating transmission](02-simular-transmissao.qmd) and [Modelling interventions](04-modelar-intervencoes.qmd)
...
The R package `{epidemics}` provides the `outcomes_averted()` function to calculate infections averted while accounting for parameter uncertainty. Let's extend our COVID-19 example from [Modelling interventions](04-modelar-intervencoes.qmd) to account for uncertainty in the basic reproduction number ($R_0$).
...
We use these parameter values alongside the population structure and contact matrix used in [Modelling interventions](04-modelar-intervencoes.qmd) to run the model for the baseline scenario:
